...
import { Radar, ArrowRight, ExternalLink, Copy, Info, Loader2 } from "lucide-react";
...
export function StealthSendTab({ wallet, network }: StealthSendTabProps) {
  ...
  // Step 1: derive the one-time stealth address from the meta-address
  const handleDerive = () => {
    setError("");
    setDerived(null);
    setTxHash("");
  ...
  // Step 2: fund the derived stealth address with a real transaction,
  // then publish the announcement so the recipient can find it.
  const handleFund = async () => {
    ...
    try {
      ...
      // sign from the wallet's first address (your funded account)
      ...
      // publish the announcement (local store, educational)
      publishAnnouncement({
        schemeId: 1,
    ...
    <div className="space-y-6">
      ...
      <div className="bg-blue-900/20 border border-blue-600/30 rounded-lg p-4 flex gap-3">
        ...
        <p className="text-xs text-blue-200">
          Paste a recipient's stealth meta-address. Their wallet stays private — this derives
          a fresh one-time address on their behalf, which you then fund with a normal transaction.
          To test, paste your <em>own</em> meta-address from the "My Address" tab.
        </p>
      </div>
          ...
          {!txHash ? (
            <div className="bg-slate-800/50 border border-slate-700 rounded-lg p-4">
              ...
              <p className="text-xs text-slate-500 mb-3">
                Funds will be sent from your Address #0 to the stealth address above, and an
                announcement will be published so the recipient can detect it.
              </p>
              <button
                onClick={handleFund}
            ...
            <div className="bg-green-900/20 border border-green-600/30 rounded-lg p-4">
              ...
              <p className="text-xs text-green-200 mb-3">
                Funded {amount} ETH and published the announcement. The recipient can now detect
                it in their Scan tab.
              </p>
              <a
                href={`${networkConfig.explorerUrl}/tx/${txHash}`}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 text-sm text-green-300 hover:text-green-200 underline"
              >
                <ExternalLink className="w-4 h-4" /> View funding tx
              </a>
              <button
                onClick={() => {
                  setDerived(null);
