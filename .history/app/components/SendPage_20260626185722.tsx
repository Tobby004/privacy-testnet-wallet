...
import { fetchGasPresets, GasOption, GasSpeed } from "@/lib/gasPresets";
...
export function SendPage({
  ...
  // Load gas presets for the current network
  useEffect(() => {
    let cancelled = false;
    (async () => {
      try {
        const opts = await fetchGasPresets(networkConfig.rpcUrls[0]);
        if (!cancelled) {
          setGasOptions(opts);
          const normal = opts.find((o) => o.speed === "normal");
          if (normal) setGasPrice(normal.gwei);
        }
      } catch {}
    })();
    return () => {
  ...
  const selectGasSpeed = (speed: GasSpeed) => {
    setGasSpeed(speed);
    const opt = gasOptions.find((o) => o.speed === speed);
    if (opt) setGasPrice(opt.gwei);
  };
    ...
    <div className="max-w-2xl">
      <div className="bg-gradient-to-br from-slate-800/50 to-slate-900/50 border border-slate-700 rounded-xl p-8">
        ...
        <form onSubmit={handleSend} className="space-y-6">
          ...
          {/* Gas presets */}
          <div>
            <label className="block text-sm font-semibold text-slate-300 mb-2">Network fee</label>
            <div className="grid grid-cols-3 gap-2 mb-2">
              {gasOptions.map((opt) => (
                <button
                  key={opt.speed}
                  type="button"
                  onClick={() => selectGasSpeed(opt.speed)}
                  className={`px-3 py-2.5 rounded-lg text-left transition border ${
                    gasSpeed === opt.speed
                      ? "bg-purple-500/20 border-purple-500/40"
                      : "bg-slate-800 border-transparent hover:bg-slate-700"
                  }`}
                >
                  <div className={`text-sm font-semibold ${gasSpeed === opt.speed ? "text-purple-300" : "text-slate-300"}`}>
                    {opt.label}
                  </div>
                  <div className="text-xs text-slate-500">{opt.gwei} Gwei</div>
                </button>
              ))}
            </div>
            <div className="flex items-center gap-2">
              ...
              <input
                ...
                step="0.1"
                min="0.1"
                className="flex-1 px-3 py-2 bg-slate-800 border border-slate-600 rounded-lg text-white placeholder-slate-500 focus:ring-2 focus:ring-purple-500 outline-none text-sm"
              />
